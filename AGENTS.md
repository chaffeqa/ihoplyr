...
## SPA Optimization
- **Output Mode**: `output: 'export'` is set in `next.config.js` to generate a static HTML export.
- **Images**: `images: { unoptimized: true }` is enabled since Vercel Image Optimization is not supported with `next export` by default without additional config, and this ensures simple static serving.
- **Vercel**: Vercel automatically detects this configuration and serves the `out/` directory as a static site.
